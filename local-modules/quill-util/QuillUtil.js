...
export default class QuillUtil {
   ...
   * `clientX` and `clientY` fields of a drop event), projects it onto
   * a Quill context, and returns information about the location of that pixel.
   *
   * The object returned will indicate the various aspects of the pixel position in the
   * Quill context. The object will have the following keys:
   ...
   *
   * If the pixel is not found in the Quill context then `POSITION_NOT_FOUND` will be returned.
   *
   * @param {Quill} quillInstance The Quill context in which to search for the pixel position
  ...
  static quillContextForPixelPosition(quillInstance, x, y) {
    ...
    // If we get a Quill instance back instead of a blot then we aren't going to find the offset
    ...
    // Binary search through the blot range looking for a bounding box that contains our pixel.
    // Tim Bray's formulation is used <https://www.tbray.org/ongoing/When/200x/2003/03/22/Binary>
    // Read that article even if you think you totally grok binary search.
    while (high - low > 1) {
      probe = (low + high) >>> 1;

      // The 1 represents a single "thing" in the blot. Could be a character or some other object
      // depending on how the blot reports its contents. Without the 1 we get a width of zero
      // which isn't helpful.
      probeBounds = quillInstance.getBounds(probe, 1);

      // We weight the search to find y-axis match first (get us on the correct line) then for an
      // x-axis match (correct lateral offset).
    ...
    ||  (y >= probeBounds.bottom)) {
      ...
      // In this case Quill reports the bounds with `left` and `right` equal to each other
      // and negative.
      const dropOnEndOfLine = (probeBounds.width === 0) && (probeBounds.left < 0);

      // Special case 2: user dropped their content way off the edge of the line, past the
      // `\n`. In that case we'll see that we're within the height of the line but all the
      // prior checks above said we're outside the lateral bounds.
      ...
      // represents offset 0 on the following line.
      ...
      // If we've gotten here then we definitely have exhausted all checks and we can't resolve
      // the drop.
      return POSITION_NOT_FOUND;
    }

    // Hooray, we found a match, and it's within the bounds of other content on the line rather
    // than past the end of the line.
    return Object.freeze({
      blot,
      blotOffset,
      line,

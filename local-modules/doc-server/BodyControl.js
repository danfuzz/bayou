...
export default class BodyControl extends CommonBase {
  ...
  async create(contents = null) {
    ...
    const maybeChange1 = [];
    if (contents !== null) {
      const change = new BodyChange(1, new BodyDelta(contents), Timestamp.now(), null);
  ...
  async deltaAfter(baseRevNum) {
    ...
    for (;;) {
      ...
      if (baseRevNum < revNum) {
        ...
        return new BodyChange(revNum, new BodyDelta(ops));
      }
  ...
  async update(baseRevNum, ops, authorId) {
    ...
    if (ops.isEmpty()) {
      return new BodyChange(baseRevNum, new BodyDelta(BodyOpList.EMPTY));
    }

    // Compose the implied expected result. This has the effect of validating
    // the contents of `delta`.
    const expected = base.compose(new BodyChange(baseRevNum + 1, new BodyDelta(ops)));
  ...
  async _applyUpdateTo(base, ops, authorId, current, expected) {
    if (base.revNum === current.revNum) {
      ...
      const change = new BodyChange(base.revNum + 1, new BodyDelta(ops), Timestamp.now(), authorId);
      ...
      return new BodyChange(revNum, new BodyDelta(BodyOpList.EMPTY));
    }
    ...
    if (dNext.isEmpty()) {
      ...
      return new BodyChange(rCurrent.revNum, new BodyDelta(BodyOpList.EMPTY));
    }

    // (3)

    const rNextNum     = rCurrent.revNum + 1;
    const change       = new BodyChange(rNextNum, new BodyDelta(dNext), Timestamp.now(), authorId);
